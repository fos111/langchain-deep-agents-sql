...
Enable verbose debug output with LangChain-style tracing:

```bash
python agent.py --debug "What are the top 5 best-selling artists?"
```

The trace output shows step-by-step execution:
- Step number, operation name, and duration
- For LLM calls: request (prompt) and response
- For tool calls: input arguments and output
...
[01] LLM                  3.78s
    → System: You are a Deep Agent...
    ← [list_tables]

[02] list_tables        0.12s
    input: None
    output: albums|artists|customers|...

[03] LLM                  2.44s
    → Based on the schema...
    ← [execute_query]

[04] execute_query     0.04s
...
----------------------------------------
Total: 10.5s | Tokens: 450/120/570 | Tools: 2
```

**What each column means:**
- `[##]` - Step number in execution order
- `LLM` - Language model call (request/response)
- `list_tables`, `execute_query` - Tool calls
- `→` - Input sent to LLM or tool
- `←` - Output from LLM (content or tool call) or tool result
- `tokens` - Input/output/total tokens used
- `Total` - End-to-end execution time

### Benchmark Mode

Run multiple queries and get a performance report:

```bash
python agent.py --benchmark
```

Run with custom queries from a JSON file:

```bash
python agent.py --benchmark --benchmark-queries my-queries.json
```

Save results to JSON:

```bash
python agent.py --benchmark --benchmark-output report.json
```

Benchmark report includes:
- Per-query: question, execution time, tokens, success/failure
- Summary: total queries, success rate, avg time, total tokens
...
You can also use the `TraceCallbackHandler` directly in your code:

```python
from agent import create_sql_deep_agent, TraceCallbackHandler

callback = TraceCallbackHandler()
agent = create_sql_deep_agent()

result = agent.invoke(
    {"messages": [{"role": "user", "content": "How many customers from Canada?"}]},
    config={"callbacks": [callback]},
)

# Access trace events
for event in callback.trace_events:
    print(f"[{event['step']}] {event['name']}: {event['duration']:.2f}s")
    if event.get("request"):
        print(f"  Request: {event['request'][:100]}...")
